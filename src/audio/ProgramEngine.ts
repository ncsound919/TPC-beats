...
export class ProgramEngine {
  ...
  public triggerPad(padId: number, velocity: number = 127, time?: number): void {
    ...
    if (pad.linkedPadIds?.length) {
      for (const linkedId of pad.linkedPadIds) {
        const linkedPad = this.program.pads.find((p) => p.padId === linkedId);
        if (linkedPad) {
          this.executeTrigger(linkedPad, velocity, time);
        }
      }
    }
    this.executeTrigger(pad, velocity, time);
  }

  private executeTrigger(pad: Pad, velocity: number, time?: number): void {
  ...
  private playLayer(layer: PadLayer, velocity: number, pad: Pad, time?: number): boolean {
    for (const sample of this.program.samples) {
      ...
      if (pad.filter?.enabled) {
        mergedSlice.filter = {
          cutoff: pad.filter.cutoff,
          resonance: pad.filter.resonance,
        };
      }

      if (pad.ampEnv) {
        mergedSlice.attack = pad.ampEnv.attack;
        mergedSlice.decay = pad.ampEnv.decay;
        mergedSlice.sustain = pad.ampEnv.sustain;
        mergedSlice.release = pad.ampEnv.release;
      }

      if (pad.filterEnv) {
        if (!mergedSlice.filter) {
          mergedSlice.filter = { cutoff: 20000, resonance: 0 };
        }
      }
