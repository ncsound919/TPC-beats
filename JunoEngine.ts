...
class JunoVoice {
  ...
  private build() {
    ...
    const detuneCents = (dco.detune || 0) - 50; // shift to center-ish if needed, or raw

    // Create DCO components
    if (dco.wavePulse) {
      this.pulseOsc = this.ctx.createOscillator();
      this.pulseOsc.type = 'square';
      this.pulseOsc.frequency.setValueAtTime(baseFreq, this.now);
      this.pulseOsc.detune.setValueAtTime(detuneCents, this.now);
      this.pulseGain = this.ctx.createGain();
      // PWM simulation using standard LFO or random modulation
      const pwmDepth = dco.pwm / 100;
      if (pwmDepth > 0) {
        this.pulseOsc.detune.setValueAtTime(detuneCents - (pwmDepth * 50), this.now);
        this.pulseOsc.detune.linearRampToValueAtTime(detuneCents + (pwmDepth * 50), this.now + 0.1);
      }
      this.pulseOsc.connect(this.pulseGain);
      this.pulseGain.connect(this.voiceMix);
      this.pulseGain.gain.setValueAtTime(0.5, this.now);
    }

    if (dco.waveSaw) {
      this.sawOsc = this.ctx.createOscillator();
      this.sawOsc.type = 'sawtooth';
      this.sawOsc.frequency.setValueAtTime(baseFreq, this.now);
      this.sawOsc.detune.setValueAtTime(detuneCents, this.now);
      this.sawGain = this.ctx.createGain();
      this.sawOsc.connect(this.sawGain);
      this.sawGain.connect(this.voiceMix);
      this.sawGain.gain.setValueAtTime(0.5, this.now);
    }

    if (dco.waveSub) {
      this.subOsc = this.ctx.createOscillator();
      this.subOsc.type = 'square';
      this.subOsc.frequency.setValueAtTime(baseFreq / 2, this.now); // sub is 1 octave lower
      this.subOsc.detune.setValueAtTime(detuneCents, this.now);
      this.subGain = this.ctx.createGain();
      this.subOsc.connect(this.subGain);
      this.subGain.connect(this.voiceMix);
      this.subGain.gain.setValueAtTime(dco.sub / 100, this.now);
    }
